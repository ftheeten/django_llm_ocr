...
class OcrQwen7():
    model_path="/opt/panafgeo_ict/src/local_models/qwen_7b_vl"
    device_type="cuda" # cpu/cuda
    
    def __init__(self,  p_prompt_1="Transcribe the text of the image like you were an OCR engine. The document is handwritten in French and concerns geology of Central Africa.", p_max_tokens=4092):        
    ...
    def run_model(self, image_paths,  p_timeout): 
        ...
        print(OcrQwen7.model_path)

        
        print("model")        
        model = Qwen2VLForConditionalGeneration.from_pretrained(
            OcrQwen7.model_path, torch_dtype="auto", device_map="auto",
            quantization_config=quant_config,
            attn_implementation="flash_attention_2",
        )
        print("processor")
        processor = AutoProcessor.from_pretrained(OcrQwen7.model_path)
        #model.to(torch.bfloat16)
        #model = model.to(OcrQwen7.device_type)
        #model.eval()        
        #generation_config = GenerationConfig().from_pretrained( OcrQwen7.model_path)
        print("loop images")
