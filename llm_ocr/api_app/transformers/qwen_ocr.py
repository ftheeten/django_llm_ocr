...
class QwenOcr():
    ...
    def __init__(self, p_uuid,  p_img, p_logger,p_timeout=60 ):
        try:
            ...
            """
            if QwenOcr.processor is None and QwenOcr.model is None and QwenOcr.generation_config is None and QwenOcr.conversation is None:
                QwenOcr.processor=AutoProcessor.from_pretrained("Qwen/Qwen2-VL-7B-Instruct")
                QwenOcr.model=Qwen2VLForConditionalGeneration.from_pretrained("Qwen/Qwen2-VL-7B-Instruct", torch_dtype="auto", device_map="cuda")
                QwenOcr.model = self.model.to("cuda") # or cuda
                QwenOcr.generation_config= GenerationConfig().from_pretrained("Qwen/Qwen2-VL-7B-Instruct")
                QwenOcr.conversation = [
    ...
    def process(self):
        try:
            ...
            inputs = QwenOcr.processor(text=[text_prompt], images=[self.img], padding=True, return_tensors="pt")
