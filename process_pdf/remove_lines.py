    ...
    def blur_vline(self): 
        try:
            ...
            if proportion_line>self.threshold_line:
                ...
                (rows, cols) = np.where(smooth != 0)
                sum_line=np.sum(smooth != 0)
                proportion_line=sum_line/(src.shape[0]*src.shape[1])
                if proportion_line>self.threshold_line:
                    return False, src
                else:
                    print("fallback smoothed")
                    print(proportion_line)
...
def main():
    global IMG
    src = cv2.imread(IMG)
    src, gray2=blur_vline(src, 2,2, False)
    cv2.imwrite(IMG+"_hor_removed.jpg", src)
    cv2.imwrite(IMG+"_hor_removed_gray.jpg", gray2)
